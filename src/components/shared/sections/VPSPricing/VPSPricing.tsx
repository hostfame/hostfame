  ...
  return (
    <section ref={sectionRef}>
      ...
      <div className="hidden lg:block">
        ...
        <div ref={wrapperRef} className="overflow-auto rounded-xl border border-border relative">
          <table ref={tableRef} className="min-w-full border-collapse relative">
            ...
            <tbody>
              {features.map((feat, rowIdx) => (
                <tr key={feat.key} className={rowIdx % 2 === 0 ? "bg-card" : ""}>
      ...
      <div className="block lg:hidden space-y-6">
        {VPSPlans.map((plan) => (
          <article key={plan.name} className="rounded-xl overflow-hidden border border-border bg-white shadow-sm">
            {/* Card header */}
            <div className="bg-primary text-primary-foreground p-4">
              <div className="text-center text-xl font-semibold">{plan.name}</div>
            ...
            <div className="p-4 space-y-3">
              {features.map((f) => {
                if (f.boolean) {
                  ...
                  return (
                    <div key={f.key} className="flex items-center justify-between bg-gray-50 rounded-md p-3 border border-border">
                      <div className="text-sm text-color-description-text">{f.label}</div>
                      <div className="flex items-center justify-center">
                        {boolVal ? (
                          <Check className="h-5 w-5 text-green-500" />
                        ) : (
